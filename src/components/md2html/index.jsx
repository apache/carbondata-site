...
const Md2Html = ComposeComponent => class extends ComposeComponent {
  ...
  handleRelativeLink() {
    ...
    // rootPath/zh-cn/docs/dir/hello.html => /docs/zh-cn/dir
  ...
  handleDocsLatestLink() {
    ...
    // rootPath/zh-cn/docs/1.3.5/user_doc/cluster-deployment.html => rootPath/zh-cn/docs/latest/user_doc/cluster-deployment.html
    const alinks = Array.from(this.markdownContainer.querySelectorAll('a'));
    alinks.forEach((alink) => {
      const href = alink.getAttribute('href');
    ...
    let innerHTML = this.markdownContainer.innerHTML.replaceAll(`apache/carbondata:${siteConfig.docsLatest}`, 'apache/carbondata:latest');
    innerHTML = innerHTML.replaceAll('$ docker tag apache/carbondata:latest apache/carbondata:latest\n', '').replaceAll(` --set image.tag=${siteConfig.docsLatest}`, '');
